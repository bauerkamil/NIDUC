...
class Menu:
    ...
    def _send_(self):
        ...
        if answear: print("Odpowiedz: Zaakceptowano")
        else: 
    ...
    def _addNoise_(self):       #zaklocenia
        if self.choice2==1:
            for i in range(0, numpy.shape(self.chanel.packets)[0]):
                self.packets[i] = self.chanel.BSC(self.chanel.p, self.chanel.packets[i])

        if self.choice2==2:
            for i in range(0, numpy.shape(self.chanel.packets)[0]):
                self.packets[i] = self.chanel.BEC(self.chanel.p, self.chanel.packets[i])
